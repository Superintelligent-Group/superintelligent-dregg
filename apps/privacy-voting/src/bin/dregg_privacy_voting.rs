//! Standalone dregg-privacy-voting server binary.

use dregg_app_framework::blinded_endpoint::FairDistributionEndpoint;
use dregg_app_framework::server::{AppConfig, AppServer};
use dregg_privacy_voting::EligibilityAuthority;
use dregg_privacy_voting::server::{AppState, router};
...
async fn main() {
    ...
    // accept `DREGG_VOTING_ISSUER_PK` (hex-encoded 32 bytes), falling back to
    ...
    let issuer_pk_bytes: [u8; 32] = std::env::var("DREGG_VOTING_ISSUER_PK")
        .ok()
        .and_then(|s| dregg_app_framework::hex::hex_to_bytes32(&s).ok())
        .unwrap_or([0xCAu8; 32]);
    ...
    AppServer::new(config)
        .service_name("dregg-privacy-voting")
        .with_health()
        .with_cors()
