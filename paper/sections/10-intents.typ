...
Traditional order books match pairs: buyer meets seller. Pyana's intent engine generalizes this to _constraint satisfaction over arbitrary capability exchanges_. An intent declares what a cell wants and what it offers; the solver finds valid combinations---including multi-party ring trades where no bilateral match exists.
...
#figure(
  ...
  caption: [Trustless protocol comparison. Pyana replaces hardware trust (SGX), reputation (CoW), and visibility (Anoma) with cryptographic proofs.],
)
