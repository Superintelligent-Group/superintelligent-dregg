...
//! `dregg_storage::blinded::NoteSpendingAir` adapted against the
...
pub const BLINDED_QUEUE_SPEND_AIR_VK: [u8; 32] = *b"dregg-storage-tpl-blindq-spendvk";
...
/// see `dregg_app_framework::AppCipherclerk` for the witness-attach API.
...
pub fn register(ctx: &StarbridgeAppContext) -> [u8; 32] {
    ...
    ctx.register_inspector(InspectorDescriptor {
        ...
        descriptor: serde_json::json!({
            "component": "dregg-blinded-queue",
            "module": "/dregg-storage-templates/blinded-queue.js",
...
mod tests {
    ...
    fn descriptor_validates_against_canonical_program() {
        ...
        dregg_app_framework::validate_child_vk_canonical(&d, &blinded_queue_program())
    ...
    fn consume_case_carries_witnessed_predicate() {
        ...
        let has_witnessed = consume.constraints.iter().any(|c| {
            matches!(c, StateConstraint::Witnessed { wp } if matches!(
                wp.kind,
                dregg_cell::predicate::WitnessedPredicateKind::Custom { vk_hash }
