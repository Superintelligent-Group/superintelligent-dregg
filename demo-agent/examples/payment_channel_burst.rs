...
fn main() {
    ...
    let alice_silo: [u8; 32] = { let mut s = [0u8; 32]; s[0] = 0xAA; s };
    let bob_silo: [u8; 32] = { let mut s = [0u8; 32]; s[0] = 0xBB; s };
    let witness_1: [u8; 32] = { let mut s = [0u8; 32]; s[0] = 0xC1; s };
    let witness_2: [u8; 32] = { let mut s = [0u8; 32]; s[0] = 0xC2; s };
    ...
    println!("    Slice ceiling: {} per silo (formula: balance * (f+1)/(2f+1) = {} * 2/3)",
             slice_ceiling, channel_deposit);
    println!("    Alice can spend up to {} computrons per epoch without consensus", slice_ceiling);
    println!();
    ...
    for seq in 1..=num_payments {
        ...
        coordinator.try_debit(alice_silo, payment_amount, digest)
    ...
    println!("  │                                                              │");
    println!("  │  Total time:   {:>10.2} us ({:.4}ms)                      │",
             burst_time.as_micros(), burst_time.as_secs_f64() * 1000.0);
    println!("  │  Per payment:  {:>10.2} us                                  │",
             per_payment.as_nanos() as f64 / 1000.0);
    println!("  │  Throughput:   {:>10.0} payments/sec                        │",
    ...
    println!("    Payment  #1: 10 computrons | Alice remaining: {}",
             slice_ceiling - 10);
    println!("    Payment #50: 10 computrons | Alice remaining: {}",
             slice_ceiling - 500);
    println!("    Payment #100: 10 computrons | Alice remaining: {}",
    ...
    println!("  Receipt chain: {} receipts linked [VALID]", receipt_chain.len());
    println!("  Monotonicity: cumulative total never decreases [VALID]");
    println!("  Bounds: total {} <= ceiling {} [VALID]", total_sent, slice_ceiling);
    println!("  Verification time: {:.2}ms (all 100 receipts)", verify_time.as_secs_f64() * 1000.0);
    println!();
    ...
    println!("  Total spent so far: {} computrons", coordinator.total_spent());
    println!();

    // Try to spend more than remaining
    let overspend_amount = remaining + 1;
    println!("  Attempting overspend: {} computrons (1 more than remaining)...", overspend_amount);
    ...
    let alice_cert = coordinator.silo_states[&alice_silo].certificate(alice_silo, &alice_signing_key);
    ...
    println!("    Alice silo:   {} spent, {} debits", alice_cert.total_spent, alice_cert.debits.len());
    ...
    let epoch_spent = coordinator.rebalance(&[alice_cert, bob_cert, w1_cert, w2_cert])
    ...
    println!("    New balance: {} computrons (was {})", coordinator.total_balance, channel_deposit);
    println!("    New version: {} (epoch {})", coordinator.version, coordinator.version);
    println!("    New slice ceiling: {} (based on new balance)", coordinator.compute_slice_ceiling());
    println!("    Rebalance time: {:.3}ms", rebalance_time.as_secs_f64() * 1000.0);
    println!();

    // Verify conservation
    assert_eq!(epoch_spent + coordinator.total_balance, channel_deposit);
    println!("  Conservation: {} (spent) + {} (remaining) = {} (deposit) [VERIFIED]",
    ...
    println!("  Alice can now spend up to {} more computrons (from remaining {}).",
    ...
    println!("  10 more payments in epoch 2: {:.2}us total ({:.2}us each)",
             epoch2_time.as_micros() as f64,
             epoch2_time.as_nanos() as f64 / 10000.0);
    println!("  Running total sent: {} computrons (110 payments)",
             total_sent + 100);
    println!();
    ...
    println!("  ├──────────────────────────────────────────────────────────────────┤");
    println!("  │  Original deposit:      {:>8} computrons                       │", channel_deposit);
    println!("  │  Epoch 1 spent:         {:>8} computrons (100 payments)        │", epoch_spent);
    println!("  │  Epoch 2 spent so far:  {:>8} computrons (10 payments)         │", 100);
    println!("  │  Alice remaining (silo):{:>8} computrons                       │", final_alice_remaining);
    println!("  │  Total balance:         {:>8} computrons                       │", coordinator.total_balance);
    println!("  │  Budget version:        {:>8} (epoch {})                        │", coordinator.version, coordinator.version);
    println!("  └──────────────────────────────────────────────────────────────────┘");
    println!();
    ...
    println!("  ├──────────────────────────────────────────────────────────────────┤");
    println!("  │  100 payments (burst)    │ {:>7.2}us     │ vs ~60,000ms on-chain  │", burst_time.as_micros() as f64);
    println!("  │  Per payment             │ {:>7.2}us     │ vs ~600ms on-chain     │", per_payment.as_nanos() as f64 / 1000.0);
    println!("  │  100 receipt verifications│ {:>7.2}ms     │ offline, no network    │", verify_time.as_secs_f64() * 1000.0);
    println!("  │  Epoch rebalance         │ {:>7.3}ms     │ one consensus op       │", rebalance_time.as_secs_f64() * 1000.0);
    println!("  │  Total demo              │ {:>7.2}ms     │                        │", total_time.as_secs_f64() * 1000.0);
    println!("  └──────────────────────────────────────────────────────────────────┘");
    println!();
    ...
    println!("  we do 100 LOCAL decrements ({:.0}us total) + 1 rebalance ({:.3}ms).",
             burst_time.as_micros() as f64, rebalance_time.as_secs_f64() * 1000.0);
    println!();
    println!("  Properties:");
