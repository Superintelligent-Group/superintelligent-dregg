...
4. The `dregg` Cipherclerk icon appears in the toolbar
...
- `page.js` — Defines `window.dregg` API in page context
- `popup.html` + `popup-script.js` — Extension popup UI

## Page API

```js
// Check if dregg cipherclerk is available
const connected = await window.dregg.isConnected();

// Request authorization
const result = await window.dregg.authorize({
  action: 'read',
  resource: '/data/x',
...
const caps = await window.dregg.getCapabilities();
```

## WASM integration (planned)

The crypto stubs in `background.js` will be replaced with calls to the dregg WASM
