...
fn bench_note_spending(c: &mut Criterion) {
    ...
    group.bench_function("verify_d4", |b| {
        b.iter(|| black_box(verify_note_spend(nullifier, merkle_root, &proof).unwrap()));
    });
    ...
    group.bench_function("verify_d8", |b| {
        b.iter(|| black_box(verify_note_spend(null8, root8, &proof8).unwrap()));
    });
...
fn build_test_revocation_tree(num_revoked: usize) -> SortedRevocationTree {
...
fn bench_non_revocation(c: &mut Criterion) {
    ...
    for &num_ancestors in &[1, 4, 8] {
        ...
        group.bench_with_input(
            ...
            &(ancestor_hashes.clone(), &tree),
            |b, (hashes, t)| {
                b.iter(|| black_box(prove_non_revocation(hashes, t).unwrap()));
            },
        );
        ...
        group.bench_with_input(
            ...
            &(revocation_root, proof.clone()),
            |b, (root, p)| {
                b.iter(|| black_box(verify_non_revocation(*root, p).unwrap()));
            },
        );
